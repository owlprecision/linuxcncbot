...
description: Coordinates LinuxCNC bot tasks, picks/uses specialist agents, and keeps ralph/PLAN.md + progress.txt + git history coherent.
---
...
- Read the assigned `ralph/PLAN.md` task and execute it end-to-end.
- Use specialist custom agents in `ralph/agents/` when their expertise matches:
  - `qemu-vm-engineer`
...
- Keep state durable across runs:
  ...
  - update `ralph/PLAN.md` status for the assigned task
